...
Proof.
  intros p. rewrite <- snd_fst_is_swap. destruct p as [n m]. reflexivity. Qed.
(** [] *)
...
Theorem add_inc_count : forall b : bag,
  count 1 ( 1 :: b ) = count 1 b + 1.
Proof.
...
Proof.
  induction l1.
 - reflexivity.
 - intros. simpl. rewrite IHl1. destruct n.
   + reflexivity.
   + reflexivity.
Qed.
(** [] *)
...
Fixpoint eqblist (l1 l2 : natlist) : bool := 
  ...
  | l1, nil => false
  | nil, l2 => false
  | h1 :: l1', h2 :: l2' => match eqb h1 h2 with
                            | true => eqblist l1' l2'
                            | false => false 
                            end
  end.
...
Proof.
  induction l.
  - reflexivity.
  - simpl. replace (n =? n) with (true). rewrite IHl. reflexivity.
    + induction n.
      * reflexivity.
...
Proof.
  ...
  - simpl. destruct n.
    ...
    + replace (S n =? 0) with (false). simpl. rewrite IHs. reflexivity.
      * reflexivity.
Qed.
(** [] *)
